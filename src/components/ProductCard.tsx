...
export default function ProductCard({ product, brandRating }: { product: Product; brandRating?: number }) {
  return (
    <Link href={`/product/${product.id}`} className="group flex flex-col overflow-hidden rounded-2xl bg-[var(--color-surface)]">
      <div className="relative aspect-[8/5] w-full overflow-hidden rounded-2xl bg-[var(--color-surface-2)]">
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="flex flex-col gap-0.5 px-0.5 pt-1.5">
        <span className="text-[9px] font-bold uppercase tracking-wide text-[var(--color-ink-faint)] truncate">{product.brandName}</span>
        ...
        {brandRating !== undefined && (
          <span className="mt-0.5 inline-flex w-fit items-center gap-1 rounded bg-[var(--color-verified-dim)] px-1.5 py-0.5 text-[9px] font-bold text-[var(--color-verified)]">
