...
const DEFAULT_THEME: SiteTheme = "tradeindia";
...
/** Sitewide theme — swaps the `--color-brand*` tokens everything in the app already reads
    ...
    project. Defaults to TradeIndia's blue (matching the server-rendered attribute in
    layout.tsx); persists any explicit choice in localStorage so it survives navigation and
    reloads. */
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // state must start at DEFAULT_THEME on both server and client so the very first client
    // render matches SSR exactly (no hydration mismatch) — the real value is applied a tick
    // later, by which point the no-flash inline script in layout.tsx has already painted the
    // right colors at the DOM level regardless of this component's state.
